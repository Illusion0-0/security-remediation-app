...
Env vars (all optional — if missing, emails are silently skipped):
    SMTP_HOST - SMTP server (e.g., smtp.gmail.com)
    SMTP_PORT - Port (default 587)
    SMTP_USER - Username (e.g., your-email@gmail.com)
    SMTP_PASS - Password or app-specific password
    EMAIL_TO - Recipient (default: same as SMTP_USER)
    EMAIL_FROM - From name/email (default: dbSecureRemediate <SMTP_USER>)
"""
...
# Brand colors matching the dashboard UI
_COLORS = {
    "Critical": "#dc2626",
    "High": "#ea580c",
    "Medium": "#ca8a04",
...
def _build_html(run: RunRecord) -> str:
    """Build branded HTML email template for the remediation report."""
  ...
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    ...
    <div style="background:#fff;padding:24px;border-radius:0 0 12px 12px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
      ...
      <div style="margin-top:16px;padding-top:12px;border-top:1px solid #eee;font-size:12px;color:#888;">
        ...
        <p style="margin:2px 0;"><strong>Status:</strong> {run.status.value if hasattr(run.status,'value') else run.status}</p>
      </div>
    </div>

    <!-- Footer -->
    <p style="text-align:center;color:#999;font-size:11px;margin-top:12px;">Sent by dbSecureRemediate · Automated Security Remediation Platform</p>
  </div>
</body>
</html>"""
...
def _send_smtp(run: RunRecord) -> None:
    """Send the email via SMTP (blocking). Called from background thread."""
    try:
...
def send_pr_notification(run: RunRecord) -> None:
    """Send HTML email notification after PR creation.

    Non-blocking: runs SMTP in a background thread.
    Fails silently if SMTP env vars are not configured.
    """
    if not _is_configured():
        logger.debug("SMTP not configured, skipping email for run %s", run.id)
        return

    # Send in background thread so it never blocks the orchestrator
    t = threading.Thread(target=_send_smtp, args=(run,), daemon=True)
    t.start()